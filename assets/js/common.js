...
$(this).addClass("on");
    ...
    var swiper = new Swiper(".main_slider .mySwiper", {
		...
		$(document).ready(function () {
				$('.main_slider .swiper-slide').eq(0).find('.text_box').addClass('on');
		});
		...
		swiper.on('slideChange', function () {
			...
			// 2초 후에 클래스를 추가합니다.
		setTimeout(function () {
			$('.main_slider .swiper-slide').eq(currentSlideIndex).find('.text_box').addClass('on');
		}, 1000); // 2초를 나타내는 2000 밀리초
			$('.main_slider .swiper-slide').eq(previousSlideIndex).find('.text_box').removeClass('on');
		});
