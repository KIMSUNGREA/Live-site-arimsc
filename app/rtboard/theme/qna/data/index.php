...
switch ($_def['func'])
{
	case "list"	:
		...
		if ($cls_board->list_rec_cnt > 0)
		{
			...
			for ($i = 0; $i < count($_l); $i++)
			{
				...
				$_l[$i]['viewpage'] = "?appcode=rtboard&sd=theme-".$_bdinfo['theme']."-data&cf=reply&bcode=".$env->_get['bcode']."&seq=".$_l[$i]['seq']."&pg=".$env->_get['pg'].$add_sch_url;
	...
	case "reply"	:
		...
		if($env->_get['seq']){
			...
			${"give_en_".$r['give_en']} = "checked='checked'";
