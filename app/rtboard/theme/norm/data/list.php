...
<div class="rt_search_wrap rt_tar">
	...
	<?php if ($env->_get['best']!="y") { ?>
	<div class="rt_button_wrap rt_fll">
		<a href="?appcode=rtboard&sd=theme-norm-data&bcode=review&ctgr=포토후기" style="<?php echo ($env->_get['ctgr']=="포토후기")?"background-color:orange;color:#fff":"";?>;">포토후기</a>
		<a href="?appcode=rtboard&sd=theme-norm-data&bcode=review&ctgr=SNS후기" style="<?php echo ($env->_get['ctgr']=="SNS후기")?"background-color:orange;color:#fff":"";?>;">SNS후기</a>
	</div>
	<?php } ?>
	<select name="ctgr">
		<option value="">분류 전체</option>
		<option value="포토후기" <?php echo ($env->_get['ctgr']=="포토후기")?"selected":"";?>>포토후기</option>
		<option value="SNS후기" <?php echo ($env->_get['ctgr']=="SNS후기")?"selected":"";?>>SNS후기</option>
	</select>
	<select name="search">
	...
	<tbody>
		...
		?>
			<tr>
				...
				<td><p><?php echo ($_l[$i]['auth_en']=="y")?"<span class='rt_blue'>승인</span>":"<span class='rt_red'>대기</span>";?></p></td>
				<?if ($_conf['category_is'] == "y") {?>
...
<div class="rt_button_wrap clearfix">
	<?php if ($env->_get['best'] == "y") { ?>
	<a href="javascript:select_status('c');" class="rt_btn_blue">선택 베스트후기 취소</a>
	<?php } else { ?>
	<a href="javascript:select_status('a');" class="rt_btn_blue">선택 승인</a>
	<a href="javascript:select_status('c');" class="rt_btn_blue">선택 베스트후기</a>
	<!-- <a href="javascript:select_status();" class="rt_btn_blue">상태변경(토글)</a> -->
...
<form name="status_form" method="post" action="<?php echo $_def['path_app'];?>/<?php echo $__sd?>/update.php" method="post" target="rt_ifrm">
	<input type="hidden" name="mode" value="select_status">
