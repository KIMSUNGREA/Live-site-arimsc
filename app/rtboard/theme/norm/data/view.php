...
<colgroup>
	...
	<tr>
		<th><p>BEST후기</p></th>
		<td><p><?php echo ($r['best_en']=="y")?"설정":"미설정";?></p></td>
	</tr>
	...
	<tr>
		<th><p>제품모델</th>
		<td><p><?php echo $r['pdt_model'];?></p></td>
	</tr>
	<tr>
		<th><p>시리얼넘버</th>
		<td><p><?php echo $r['serial_num'];?></p></td>
	</tr>
	<tr>
		<th><p>추천인코드</th>
		<td><p><?php echo $r['recode'];?></p></td>
	</tr>
	<tr>
		<th><p>승인여부</th>
		<td><p><?php echo ($r['auth_en']=="y")?"승인":"대기";?></p></td>
	</tr>
	<tr>
		<th><p>SNS URL</th>
		<td><p><a href="<?php echo $r['sns_url'];?>" target="_blank"><?php echo $r['sns_url'];?></a></p></td>
	</tr>
	<!-- <tr>
		<th><p>상품지급</th>
		<td><p><?php echo ($r['auth_en']=="y")?"완료":"미지급";?></p></td>
	</tr> -->
	<?php
	if (count($attc_arr) > 0) {
		for ($m = 0; $m < count($attc_arr); $m++) {
...
<div class="rt_button_wrap rt_tar mb25">
	<a href="javascript:view_status();" class="rt_btn_blue">
		<?php echo ($r['auth_en']=="y")?"승인 취소":"승인";?>
	</a>
...
<form name="status_form" method="post" action="<?php echo $_def['path_app'];?>/<?php echo $__sd?>/update.php" method="post" target="rt_ifrm">
	...
	<input type="hidden" name="div" value="a">
	<input type="hidden" name="seqstr" value="<?php echo $r['seq'];?>/">
</form>

<script src="<?php echo $_def['path_section'];?>/js/rtboard.theme.norm.data.js?ver=001" type="text/javascript"></script>
