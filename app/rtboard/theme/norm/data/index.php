...
if ($env->_get['ctgr']) {
	$add_qry[] = "category='".$env->_get['ctgr']."'";
	$add_url.= "&ctgr=".$env->_get['ctgr'];
	$add_sch_url = "&ctgr=".$env->_get['ctgr'];
}
...
switch ($_def['func'])
{
	case "list"	:
		...
		if ($cls_board->list_rec_cnt > 0)
		{
			...
			$_l = $cls_board->get_list("auth_en DESC,seq DESC");
	...
	case "form"	:
		...
		if($env->_get['seq']){
			...
			${"auth_en_".$r['auth_en']} = "checked";
			${"best_en_".$r['best_en']} = "checked";
